...
describe("Test utils.js", () => {
    ...
    test("test getFileName", function () {
        ...
        expect(utils.getFileName("https://domain.com/path/abc.mp4?abc=123", "jpg", "filename.mp4")).toBe("filename.mp4");
    });
    ...
    describe("isNew", () => {
        ...
        test("returns false if list is not an array", () => {
            expect(() => { isNew(null, "a") }).toThrow();
            expect(() => { isNew({}, "a") }).toThrow();
        });

        describe("primitive values", () => {
            test("returns true if value not in list", () => {
                expect(isNew([1, 2, 3], 4)).toBe(true);
            });

            test("returns false if value already in list", () => {
                expect(isNew([1, 2, 3], 2)).toBe(false);
            });

            test("uses strict equality", () => {
                expect(isNew(["1"], 1)).toBe(true);
            });
        });

        describe("object values with url", () => {
            test("returns true when url is not present", () => {
                const list = [{ url: "a" }, { url: "b" }];
                expect(isNew(list, { url: "c" })).toBe(true);
            });

            test("returns false when url already exists", () => {
                const list = [{ url: "a" }, { url: "b" }];
                expect(isNew(list, { url: "b" })).toBe(false);
            });

            test("ignores other object properties", () => {
                const list = [{ url: "a", x: 1 }];
                expect(isNew(list, { url: "a", x: 999 })).toBe(false);
            });
        });

        describe("object values without url", () => {
            test("falls back to reference equality", () => {
                const obj = { a: 1 };
                expect(isNew([obj], obj)).toBe(false);
                expect(isNew([obj], { a: 1 })).toBe(true);
            });
        });

        test("handles mixed lists safely", () => {
            const list = [{ url: "x" }, "a", 1];
            expect(isNew(list, { url: "x" })).toBe(false);
            expect(isNew(list, "a")).toBe(false);
            expect(isNew(list, 2)).toBe(true);
        });
    });
});
